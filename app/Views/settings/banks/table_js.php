if ($("#tblBank").length && tabClicked === "tab-bank") {
    if(typeof(dTable['tblBank'])!=='undefined'){
        $(".settings").removeClass("active");
        $("#tab-bank").addClass("active");
        dTable['tblBank'].ajax.reload(null,true);
        counter=1;
   }else{
    counter=1;
    dTable['tblBank'] = $('#tblBank').DataTable( {
    "pageLength": 10,
    "searching": true,
    "paging": true,
    "processing": false,
    ...
    buttons: <?php if(($_SESSION['role_id']==1) ||($_SESSION['role_id']==5)) { ?> getBtnConfig('Banks'), <?php } else { echo "[],"; } ?>
      "ajax": {
        url:"<?php echo site_url("bank.json");?>",
        dataType: 'JSON',
        type: 'POST',
        data: function(d){
         // d.status_id=1;
      ...
      ,
      { data: 'id', render:function ( data, type, full, meta ) {
        var ret_txt ="<a class='badge badge-info edit_me' data-toggle='modal' style='margin-right:10px;' href='#add_bank'><i class='fa fa-edit'></i></a>";
